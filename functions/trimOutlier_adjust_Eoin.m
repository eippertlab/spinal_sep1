...
function [EEG, artifact_info] = trimOutlier_adjust_Eoin(EEG, maxThreshold_lf, amplitudeThreshold_hf, pointSpreadWidth)

if ~(nargin==4)
    error('trimOutlier_adjust() requires 4 input arguments.')
end
...
EEG_lowfreq.data = filtfilt(a,b,double(EEG.data)')';
meanAllChan = mean(EEG_lowfreq.data, 1);
stdAllChan  = std( EEG_lowfreq.data, [], 1);
posi2SDChan = meanAllChan + 5*stdAllChan;
nega2SDChan = meanAllChan - 5*stdAllChan;
newThre = max(max(abs(nega2SDChan), posi2SDChan));

if newThre > maxThreshold_lf
    amplitudeThreshold = maxThreshold_lf;
else
...
abs_lf = abs(EEG_lowfreq.data(:,:));
bad_lf = abs_lf > amplitudeThreshold;

% Adjust parameters for higher frequency activity %
% high-pass filter
[d,c] = butter(2,[15 45]/(EEG.srate/2));
EEG_hf.data = filtfilt(d,c,double(EEG.data)')';

% compute data points
abs_hf = abs(EEG_hf.data(:,:));
bad_hf = abs_hf > amplitudeThreshold_hf;

% bad points hf and lf
sum_points = bad_lf + bad_hf;
...
if any(sum(bad_points, 2))
    ...
    disp('*****************************')
    fprintf(['LF-threshold: %2.0f' char(181) 'V; \n HF-threshold: %2.0f' char(181) 'V; \n spreading: %2.0fms; \n rejected: %2.1fsec (%2.1f %%); \n added boundaries: %2.0f; \n removed channels: %s\n'], ...
        amplitudeThreshold, ...
        amplitudeThreshold_hf, ...
        windowSize, ...
        badPointsInSec, ...
    ...
    artifact_info.hf_threshold = amplitudeThreshold_hf;
