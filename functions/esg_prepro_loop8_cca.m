...
function esg_prepro_loop8_cca(subject, nerve, srmr_nr, plot_graphs)
...
if plot_graphs
    ...
    page_path = [getenv('ZIMDIR') '03_CCAmixed_' ref_chan '_Ref/'];
    page_title = [];
...
% make sure window doesn't include interpolation time
...
if plot_graphs
    ...
    for icomp = 1:all_components
        ...
        colorbar_axes = [];
        mrmr_esg_isopotentialplot(subject, A_st(:, icomp), ...
            colorbar_axes, {epo.chanlocs.labels})
...
for icomp = 1:all_components
    ...
    if plot_graphs
        ...
        if nerve == 1
            chan_str = 'SC6';
            % title_str = 'Cervical spine';
        elseif nerve == 2
            chan_str = 'L1';
            % title_str = 'Lumbar spine';
        end
        if nerve == 1
